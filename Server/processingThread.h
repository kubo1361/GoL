...
void * prtFun(void *args) {
    ...
    while(true) {
        ...
        cout << "proccessing: id: " << data->connection->getId()  << " socket: " << data->connection->getSocket() << endl;
        pthread_mutex_unlock(&data->connection->getConnectionMediatorMut());
        ...
        if (action_full[0].compare("loadPatternNames") == 0) { // action: loadPatternNames, return: name1;name2;name3;...
            ...
            cout << temp << endl;
        } else if (action_full[0].compare("removePattern") == 0) { // action: removePattern;name, return: success
