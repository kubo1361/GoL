...
namespace structures 
{
	...
	public:
		...
		/// <summary> Move konstruktor. </summary>
		/// <param name = "other"> Pole, z ktoreho sa prevezmu vlastnosti. </param>
		Array(Array<T>&& other);
	...
	template<typename T>
	Structure& Array<T>::operator=(Structure&& other)
	{
		if (this != &other)
		{
			*this = dynamic_cast<const Array<T>&>(other);
		...
		if (this != &other)
		{
			if (size_ == other.size_) {
				*vector_ = *other.vector_;
				...
				throw std::invalid_argument("Arrays size differ"); 
			}
		}
		return *this;
	}
	...
	{
		//ziska pointer na byte vo vektore, pretypuje na T*
	...
	{
		Vector::copy(*src.vector_, src.mapFunction(srcStartIndex), *dest.vector_, dest.mapFunction(destStartIndex), sizeof(T)*length);
	}

	template<typename T>
	inline int Array<T>::mapFunction(const int index) const
	{
		//premapuje index pola na index vectora
